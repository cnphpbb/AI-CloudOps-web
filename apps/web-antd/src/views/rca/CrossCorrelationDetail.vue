<template>
  <div class="cross-correlation-detail">
    <a-page-header
      class="page-header"
      :title="`跨时滞相关分析详情 - ${jobId}`"
      sub-title="跨时滞相关性分析结果详情与数据洞察"
      @back="goBack"
    >
      <template #extra>
        <a-space>
    ...
    <div v-if="loading && !correlationDetail" class="loading-container">
      <a-spin size="large">
        ...
        <div class="loading-text">正在加载跨时滞相关分析详情...</div>
      </a-spin>
    </div>
    ...
    <div v-else-if="correlationDetail" class="detail-content">
      ...
      <a-card class="overview-card">
        ...
        <a-descriptions :column="3" bordered>
          ...
          <a-descriptions-item label="相关指标数量">
            <a-badge :count="correlations.length" show-zero>
        ...
        <a-row :gutter="24">
          ...
          <a-col :span="6">
            <a-card size="small" class="stat-item" hoverable>
              ...
              >
                <template #prefix>
                  <thunderbolt-outlined style="color: #fa8c16" />
                </template>
              </a-statistic>
      ...
      <!-- 相关性结果 -->
      <a-card title="跨时滞相关性分析结果" class="correlations-card">
        <template #title>
          <a-space>
          ...
          >
            ...
            >
              ...
              <div class="correlation-content">
                <!-- 时滞相关性显示条 -->
                <div class="lag-correlation-section">
                  <a-typography-text type="secondary" class="lag-label">
                    最优时滞：{{ getOptimalLag(result) }}
                  </a-typography-text>
      ...
      <!-- 时滞分析洞察 -->
      <a-card title="时滞分析洞察" class="insights-card">
        <template #title>
          <a-space>
            <bulb-outlined />
            时滞分析洞察
    ...
    <div v-else class="no-data">
      <a-empty description="未找到跨时滞相关分析结果">
        <template #image>
...
import { getCrossCorrelationDetailApi } from '#/api/core/rca';
import type { CorrelationAnalysisEntity } from '#/api/core/rca';

// 获取最大滞后步数（从 correlations 内部每个对象的 optimal_lag 字段中获取最大值）
function getMaxLags() {
  if (!correlationDetail.value?.correlations?.correlations) return 'N/A';
  const lags = correlationDetail.value.correlations.correlations
    .map((c: any) => typeof c.optimal_lag === 'number' ? c.optimal_lag : 0)
    .filter((lag: number) => !isNaN(lag));
  if (lags.length === 0) return 'N/A';
  return Math.max(...lags);
}

// 获取单个相关性结果的最优时滞
function getOptimalLag(result: any) {
  return typeof result.optimal_lag === 'number' ? result.optimal_lag : 0;
}

const route = useRoute();
const router = useRouter();
const jobId = ref(route.params.jobId as string || route.params.id as string);
...
const correlations = computed(() => {
  if (!correlationDetail.value?.correlations?.correlations) return [];
  return correlationDetail.value.correlations.correlations;
});

const significantCount = computed(() => {
  return correlations.value.filter(c => c.significance).length;
});

const strongCorrelations = computed(() => {
  return correlations.value.filter(c => Math.abs(c.correlation_coefficient) > 0.7).length;
});

const averageCorrelation = computed(() => {
  if (correlations.value.length === 0) return 0;
  const sum = correlations.value.reduce((acc, c) => acc + Math.abs(c.correlation_coefficient), 0);
  return sum / correlations.value.length;
});

const maxCorrelation = computed(() => {
  if (correlations.value.length === 0) return 0;
  const max = Math.max(...correlations.value.map(c => Math.abs(c.correlation_coefficient)));
  return max;
});

const sortedCorrelations = computed(() => {
  if (!correlations.value) return [];
  
  const correlationsCopy = [...correlations.value];
  
  switch (sortBy.value) {
    case 'correlation':
      return correlationsCopy.sort((a, b) => Math.abs(b.correlation_coefficient) - Math.abs(a.correlation_coefficient));
    case 'significance':
      return correlationsCopy.sort((a, b) => {
        if (a.significance && !b.significance) return -1;
        if (!a.significance && b.significance) return 1;
        return Math.abs(b.correlation_coefficient) - Math.abs(a.correlation_coefficient);
      });
    case 'name':
      return correlationsCopy.sort((a, b) => a.metric_name.localeCompare(b.metric_name));
    default:
...
const fetchDetail = async () => {
  loading.value = true;
  try {
    const response = await getCrossCorrelationDetailApi(jobId.value);
    correlationDetail.value = response.data || null;
  } catch (error) {
    console.error('Failed to load cross-correlation detail:', error);
    message.error('加载跨时滞相关分析详情失败');
  } finally {
    loading.value = false;
  }
};
...
const exportResults = () => {
  ...
  a.href = url;
  a.download = `cross-correlation-analysis-${jobId.value}-${dayjs().format('YYYY-MM-DD-HH-mm')}.json`;
  a.click();
  URL.revokeObjectURL(url);
  
  message.success('跨时滞相关分析结果已导出');
};
...
const generateInsights = () => {
  ...
  const correlationsList = correlations.value;

  if (correlationsList.length === 0) return insights;

  // 找到最强正相关
  const strongestPositive = correlationsList.reduce((max, c) =>
  ...
  if (strongestPositive && strongestPositive.correlation_coefficient > 0.5) {
    const lagInfo = '';
    insights.push(`${strongestPositive.metric_name} 与目标指标具有最强的正相关性${lagInfo}，相关系数为 ${strongestPositive.correlation_coefficient.toFixed(3)}，表明存在延迟影响关系。`);
  }

  // 找到最强负相关
  const strongestNegative = correlationsList.reduce((min, c) =>
  ...
  if (strongestNegative && strongestNegative.correlation_coefficient < -0.5) {
    const lagInfo = '';
    insights.push(`${strongestNegative.metric_name} 与目标指标具有最强的负相关性${lagInfo}，相关系数为 ${strongestNegative.correlation_coefficient.toFixed(3)}，表明存在反向延迟影响。`);
  }

  // 显著性相关性
  const nonSignificant = correlationsList.filter(c => !c.significance).length;
  if (nonSignificant > 0) {
    insights.push(`有 ${nonSignificant} 个指标与目标指标之间在考虑时滞后仍没有统计上显著的相关性，可能不存在因果关系。`);
  }

  // 平均相关性洞察
  if (averageCorrelation.value > 0.6) {
    insights.push(`平均跨时滞相关系数达到 ${averageCorrelation.value.toFixed(3)}，表明整体时滞相关性较强，系统中存在明显的延迟影响模式。`);
  } else if (averageCorrelation.value < 0.3) {
    insights.push(`平均跨时滞相关系数仅为 ${averageCorrelation.value.toFixed(3)}，说明大部分指标即使考虑时滞效应也与目标指标相关性较弱。`);
  }
