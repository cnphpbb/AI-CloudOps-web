...
onMounted(() => {
  ...
  const initAnalysisChart = () => {
    ...
    setInterval(() => {
      chart.setOption({
        series: [{
          data: Array(8).fill(0).map(() => Math.floor(30 + Math.random() * 40))
        }, {
          data: Array(8).fill(0).map(() => Math.floor(40 + Math.random() * 40))
        }, {
          data: Array(8).fill(0).map(() => Math.floor(2 + Math.random() * 8))
        }]
      });
    }, 5000);
  };
